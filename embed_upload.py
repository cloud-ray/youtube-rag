...
def get_combined_transcript(video_id, chunk_size=10):
    try:
        ...
        logger.info(f"Transcript fetched. Total chunks to create: {len(transcript) // chunk_size + 1}")
        ...
        logger.info(f"Transcript successfully chunked into {len(combined_transcript)} chunks.")
        return combined_transcript
    except TranscriptsDisabled:
        logger.error(f"Transcripts are disabled for video_id: {video_id}. No subtitles available.")
    except NoTranscriptFound:
        logger.error(f"No subtitles found for video_id: {video_id}.")
...
def embeddings_exist(video_id, vector_store):
    ...
    try:
        ...
        logger.exception(f"Error in search method: {e}")
        return False
...
def add_transcript_to_chroma(video_id, vector_store, chunk_size=10):
    ...
    for i, combined_item in enumerate(combined_transcript):
        ...
        logger.info(f"Processing chunk {i+1}/{len(combined_transcript)}: ID = {chunk_id}")
    ...
    logger.info(f"Adding {len(documents)} documents to the ChromaDB vector store...")
...
def process_video_transcript(video_id, chromadb_path="chromadb/", chunk_size=10):
    ...
    if embeddings_exist(video_id, vector_store):
        logger.info(f"Embeddings already exist for video_id: {video_id}. Skipping processing.")
    else:
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting process for video_id: {video_id}")
