...
def create_prompt_template():
    ...
    logger.debug(f"Type of template_string: {type(template_string)}")
    logger.debug(f"Content of template_string: {template_string}")
...
def run_rag_chain(user_question, retriever, prompt_template, llm, original_video_id):
    ...
    try:
        ...
        logger.exception(f"Error running RAG chain: {e}")
...
def generate_youtube_link(user_question, original_video_id):
    ...
    logger.debug(f"Original Video ID: {original_video_id}")
    logger.debug(f"Start Value (before rounding): {start_value}")
    ...
    if original_video_id and start_value is not None:
        ...
        logger.info(f"YouTube Link: {youtube_link}")  # Log the generated YouTube link
        ...
        logger.error("Unable to create YouTube link or retrieve answer.")
        return {
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting process for video_id: {original_video_id}")
    youtube_link, answer = generate_youtube_link(user_question, original_video_id)
    if youtube_link:
        logger.info(f"Answer:\n{answer}")
        logger.info(f"Check it out here:\n   {youtube_link}")
    else:
        logger.error(answer)
